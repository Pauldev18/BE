...
namespace DichVuSim_BE.Controllers
{
    ...
    public class DichVuNguoiDungController : ControllerBase
    {
        ...
        public async Task<ActionResult<IEnumerable<DichVuNguoiDungDTO>>> GetDichVuNguoiDungByMaNguoiDung(int maNguoiDung)
        {
            var dichVuNguoiDungs = await _context.DichVuNguoiDungs
                                            .Where(dv => dv.MaNguoiDung == maNguoiDung)
                                            .ToListAsync();

            if (dichVuNguoiDungs == null || dichVuNguoiDungs.Count == 0)
            {
                return NotFound("Không tìm thấy dịch vụ nào cho người dùng này.");
            }
